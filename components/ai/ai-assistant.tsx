    ...
    <Portal>
      ...
      {open && (
        <>
          ...
          <div
            className="fixed inset-0 z-[120] bg-black/30 sm:hidden"
            onClick={() => setOpen(false)}
          />

          <div
            className="fixed bottom-5 right-5 z-[121] flex flex-col w-[380px] max-w-[calc(100vw-2.5rem)] h-[600px] max-h-[calc(100vh-3rem)] overflow-hidden rounded-2xl border border-white/[0.08] shadow-[0_30px_80px_rgba(0,0,0,0.6)] page-enter"
          ...
          >
            ...
            {/* Header */}
            <div className="relative shrink-0 flex items-start justify-between gap-3 px-5 py-4 border-b border-white/[0.06]">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-[14px] font-bold text-white leading-none">Asistente GovBidder</h3>
                  <p className="text-[11px] text-white/45 mt-1">Powered by Claude</p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                {messages.length > 0 && (
                  <button
                    onClick={reset}
                    className="flex h-7 w-7 items-center justify-center rounded-lg text-white/35 hover:text-white hover:bg-white/[0.06] transition-all"
            ...
            <div ref={scrollRef} className="relative flex-1 overflow-y-auto px-5 py-4 space-y-4">
              {messages.length === 0 ? (
                <div className="flex flex-col items-center text-center pt-6 pb-2">
                  ...
                  <h4 className="text-[14px] font-bold text-white mb-1">¿En qué te ayudo?</h4>
                  ...
                  <div className="mt-5 w-full space-y-1.5">
                    {SUGGESTED.map(s => (
                      <button
                        ...
                        className="group flex w-full items-center gap-2 rounded-xl border border-white/[0.06] bg-white/[0.02] hover:border-white/[0.14] hover:bg-white/[0.05] px-3 py-2.5 text-left transition-all"
                      >
                        <Sparkles className="h-3 w-3 shrink-0 text-[#ff6b6a]/60" />
                        <span className="text-[12px] text-white/75 group-hover:text-white">{s.label}</span>
                      </button>
                    ))}
                  </div>
                </div>
            ...
            {/* Input */}
            <div className="relative shrink-0 border-t border-white/[0.06] bg-white/[0.02] p-3">
              <div className="flex items-end gap-2 rounded-xl border border-white/[0.08] bg-[#080d1e] px-3 py-2 focus-within:border-white/20 transition-colors">
                <textarea
                  ref={inputRef}
                  value={input}
                  ...
                  rows={1}
                  className="flex-1 bg-transparent text-[13px] text-white placeholder:text-white/30 outline-none resize-none max-h-[120px] py-1"
                />
                <button
                  onClick={() => send()}
  ...
  return (
    <div className={`flex items-start gap-2.5 ${isUser ? "flex-row-reverse" : ""}`}>
      ...
      <div className={`max-w-[78%] rounded-xl px-3 py-2.5 text-[13px] leading-relaxed ${
        isUser
          ? "bg-[#E42D2C]/10 text-white border border-[#E42D2C]/20"
