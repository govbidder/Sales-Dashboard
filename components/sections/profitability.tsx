...
export function Profitability() {
  ...
  if (!stats) {
    return (
      <section>
        <div className="mb-6">
          <h2 className="text-xl font-bold text-white">Rentabilidad</h2>
        ...
        <div className="rounded-2xl border border-white/[0.07] bg-[#0d1745] p-10 text-center">
          <p className="text-sm text-white/40">Cargá métricas para ver la rentabilidad.</p>
        </div>
      </section>
    )
  }
  ...
  const profitColor = cur.profit >= 0 ? "text-emerald-300" : "text-red-300"
      ...
      <div className="grid gap-3 sm:grid-cols-2 mb-4">
        <div className="rounded-2xl border border-white/[0.07] bg-[#0d1745] p-5">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-widest text-white/35">Beneficio neto</p>
            ...
            {prev && deltaProfit != null && (
              <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-bold ${
                profitIsUp ? "bg-emerald-500/10 text-emerald-300 ring-1 ring-emerald-500/20"
        ...
        <div className="rounded-2xl border border-white/[0.07] bg-[#0d1745] p-5">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-widest text-white/35">Margen</p>
          ...
          {deltaMargin != null && (
            <p className={`mt-2 text-[11px] ${deltaMargin >= 0 ? "text-emerald-300" : "text-red-300"}`}>
            ...
            return (
              <div key={c.key} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-[10px] font-bold tabular-nums text-white/50">{pct.toFixed(0)}%</span>
                </div>
                <p className="text-lg font-bold tracking-tight text-white tabular-nums">{fmtMoney(v)}</p>
                <p className="text-[11px] text-white/45">{c.label}</p>
