    ...
    <Portal>
      ...
      <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="pointer-events-none absolute -bottom-32 -left-32 h-[260px] w-[260px] rounded-full bg-[#152978]/40 blur-[80px]" />

          {/* Header with icon */}
          <div className="relative shrink-0 px-6 pt-6 pb-5 border-b border-white/[0.06]">
            <button
              type="button"
              onClick={onClose}
              className="absolute top-4 right-4 flex h-8 w-8 items-center justify-center rounded-lg text-white/35 hover:text-white hover:bg-white/[0.06] transition-all"
            >
              <X className="h-4 w-4" />
            </button>
            <div className="flex items-start gap-3">
              ...
              <div className="pt-1">
                <h3 className="text-[18px] font-bold tracking-tight text-white leading-none">
                  Invitar al equipo
                </h3>
                <p className="mt-2 text-[12px] text-white/55 leading-relaxed pr-8">
          ...
          <div className="relative flex-1 overflow-y-auto px-6 py-5 space-y-4">
            <div className="space-y-1.5">
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/35">Email *</p>
              <input
            ...
            <div className="space-y-1.5">
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/35">Nombre</p>
              <input
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <p className="text-[10px] font-bold uppercase tracking-widest text-white/35">Posición</p>
                <input
              ...
              <div className="space-y-1.5">
                <p className="text-[10px] font-bold uppercase tracking-widest text-white/35">Rol</p>
                <select
                  value={role}
            ...
            {error && (
              <div className="flex items-start gap-2 rounded-xl border border-red-500/25 bg-red-500/10 px-3 py-2.5 text-[12px] text-red-300">
          ...
          {/* Footer with action */}
          <div className="relative shrink-0 px-6 py-4 border-t border-white/[0.06] bg-white/[0.02]">
            <button
              type="submit"
              disabled={!canSubmit}
              className="group flex w-full items-center justify-center gap-2 h-11 rounded-xl bg-[#E42D2C] text-[13px] font-bold text-white shadow-[0_8px_24px_rgba(228,45,44,0.25)] hover:bg-[#c42423] hover:shadow-[0_12px_32px_rgba(228,45,44,0.40)] transition-all disabled:opacity-40 disabled:shadow-none disabled:cursor-not-allowed"
            >
              {inviting ? (
                <>
  ...
  return (
    <>
      ...
      <div className="space-y-6">
        ...
        {/* HERO ───────────────────────────────────────────────────────────── */}
        <div className="relative overflow-hidden rounded-3xl border border-white/[0.06] bg-[#0d1745]">
          <div className="pointer-events-none absolute inset-0">
            <div className="absolute -top-32 -right-32 h-[400px] w-[400px] rounded-full bg-[#E42D2C]/[0.08] blur-[100px]" />
            <div className="absolute -bottom-24 -left-24 h-[300px] w-[300px] rounded-full bg-[#152978]/40 blur-[100px]" />
          </div>
          ...
          <div className="relative grid lg:grid-cols-[1fr_auto] gap-4 p-6 sm:p-7 items-end">
            <div>
              ...
              <h1 className="text-[32px] sm:text-[40px] font-bold tracking-tight text-white leading-[1.05]">
                {activeCount === 0 ? (
                  <>Sin miembros aún</>
                ) : (
                  <>
                    <span className="text-white">{activeCount}</span>{" "}
                    <span className="text-white/85">{activeCount === 1 ? "miembro activo" : "miembros activos"}</span>
                  </>
                )}
              </h1>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                onClick={fetchTeam}
                disabled={loading}
                className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.03] text-white/55 hover:text-white hover:border-white/[0.16] hover:bg-white/[0.06] transition-all disabled:opacity-40"
              >
              ...
              {isAdmin && (
                <button
                  ...
                  className="group flex items-center gap-2 h-10 rounded-xl bg-[#E42D2C] px-4 text-[13px] font-bold text-white shadow-[0_8px_24px_rgba(228,45,44,0.25)] hover:bg-[#c42423] hover:shadow-[0_12px_32px_rgba(228,45,44,0.40)] transition-all"
                >
          ...
          {/* Stats strip */}
          <div className="relative grid grid-cols-2 sm:grid-cols-4 border-t border-white/[0.05]">
            {[
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[24px] font-bold tabular-nums text-white leading-none">
                    {s.val}
                  </p>
                </div>
              )
              ...
              <button
                ...
                className={`h-9 rounded-xl border px-3.5 text-[12px] font-medium transition-all capitalize ${
                  filterStatus === s
                    ...
                    : "border-white/[0.07] text-white/45 hover:text-white hover:border-white/20"
                }`}>
                {s}
              </button>
            ))}
        ...
        {loading ? (
          ...
          <div className="relative flex flex-col items-center justify-center overflow-hidden rounded-3xl border border-white/[0.07] bg-[#0d1745] py-20 text-center">
            ...
            <h3 className="relative text-[16px] font-bold text-white mb-1">
              {members.length === 0 ? "Sumá al primer miembro del equipo" : "Sin coincidencias"}
            </h3>
            <p className="relative max-w-sm text-[13px] text-white/45 mb-5 px-4">
              {members.length === 0
            ...
            {isAdmin && members.length === 0 && (
              <button
                onClick={() => setShowInvite(true)}
                className="group relative flex items-center gap-2 h-10 rounded-xl bg-[#E42D2C] px-4 text-[13px] font-bold text-white shadow-[0_8px_24px_rgba(228,45,44,0.25)] hover:bg-[#c42423] hover:shadow-[0_12px_32px_rgba(228,45,44,0.40)] transition-all"
              >
                <UserPlus className="h-4 w-4 group-hover:-translate-y-0.5 transition-transform" />
